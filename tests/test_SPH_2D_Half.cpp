...
int main(void)
{
	...
	string fname = "tests/test_output/test_half_timstep_" + to_string(iterations) + ".vtp";
	write_file(fname.c_str(), &(domain.particle_list));
	cout << "TEST for SPH_2D starts: 1000 iterations, Half Timestep. VTP file generated each 200 steps in tests/test_output/" << endl;
	clock_t start = clock();
	while (iterations < 1000) {
		...
		if (iterations % 200 == 0)
		{
			fname = "tests/test_output/test_half_timstep_" + to_string(iterations) + ".vtp";
			write_file(fname.c_str(), &(domain.particle_list));
		}
		iterations++;
	}
