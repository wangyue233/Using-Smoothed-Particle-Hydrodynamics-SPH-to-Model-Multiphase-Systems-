 ...
 */
	...
	// update x_next
	part->x_next[0] = part->x[0] + 0.5*dt * part->v[0];
	part->x_next[1] = part->x[1] + 0.5*dt * part->v[1];
					...
					for (unsigned int cnt = 0; cnt < search_grid[i][j].size(); cnt++)
					{
						...
						if (part != other_part)					//stops particle interacting with itself
						{
							...
							if (dist < 2. * h)					//only particle within 2h
							{
								...
								part->update_rho(part->rho_nxt, 0.5*dt, other_part, smooth, eij, vij);
								part->update_vel(part->v_next, dist, 0.5*dt, other_part, smooth, eij, vij);
...
void SPH_main::neighbour_iterate2(SPH_particle* part)					//iterates over all particles within 2h of part - can be made more efficient using a stencil and realising that all interactions are symmetric
{
/*
 * Description:
 ...
 */
	...
	// update x_next
	part->x[0] += dt * part->v_next[0];
	part->x[1] += dt * part->v_next[1];
					...
					for (unsigned int cnt = 0; cnt < search_grid[i][j].size(); cnt++)
					{
						...
						if (part != other_part)					//stops particle interacting with itself
						{
							...
							if (dist < 2. * h)					//only particle within 2h
							{
								...
								smooth = part->grad_kernel(dist / h, h);
								part->update_rho(part->rho, dt, other_part, smooth, eij, vij);
								part->update_vel(part->v, dist, dt, other_part, smooth, eij, vij);
